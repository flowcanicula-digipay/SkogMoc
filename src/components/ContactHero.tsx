...
export default function ContactHero() {
  ...
  useEffect(() => {
    ...
    // applied to. SplitType moves the title's text into nested word spans,
    // leaving the <h1> itself empty — so the gradient had nothing to clip
    // against and the title rendered invisible on every normal (non
    // reduced-motion) load. Apply the shimmer to the word spans instead,
  ...
  return (
    ...
    >
      ...
      <img
        ref={imageRef}
        src={stockImages.installation24}
        alt=""
        aria-hidden="true"
      ...
      <div className="relative z-10 mx-auto max-w-7xl text-center">
        ...
        <h1
          ref={titleRef}
          className="contact-hero-split mx-auto mt-5 max-w-3xl font-display text-[clamp(2.5rem,7vw,5.5rem)] font-extrabold leading-[1.04] tracking-tight text-amber-100"
        >
          {t('title')}
        </h1>
        <span
        ...
        <p
          ref={paragraphRef}
          className="contact-hero-split mx-auto mt-7 max-w-xl text-base leading-relaxed text-linen-50/75"
        >
          {t('subtitle')}
        </p>
      </div>
